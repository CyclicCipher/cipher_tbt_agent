...
| Phase XX | Partial evaluation / open lambda terms: `ctkg/core/lambda_term.py` introduces `LetStep` + `LambdaTerm` (explicit let-chain over `Expr` algebra); `eval_term` uses lazy streaming β-reduction (evaluates steps only as needed, returns delimiter tokens before computing step values); `synthesize_library` lifts all ops with RelationRules to LambdaTerms; Level 1b-λ in `predict.py` dispatches trace-format ops via `lambda_predict`; creative transfer handles novel ops via structural similarity (same arity + output_signature prefix, evidence-weighted) |
| Phase XXI | Dependent type inference: `ctkg/core/dependent_type.py` defines `TypeTerm` (NNO_DIGIT with ordinal, NNO_CARRY, STRUCTURAL, UNKNOWN); `infer_token_types(succ_chain)` walks the NNO successor chain and assigns ordinal-specific types; `types_compatible_under_bijection` is the formal statement of the anonymization theorem — two tokens with the same NNO ordinal are computationally indistinguishable; `RelationRule` extended with `arg1_type`/`arg2_type`/`output_type` fields (universally quantified, ordinal=None); `discover_relation_rules` accepts `type_context` and annotates discovered rules |
| Phase XXII | Probability monad and enrichment over [0,1]: `RelationRule` extended with `total_obs` field and `confidence` property (evidence/total_obs); `RelationRule.evaluate()` returns `dict[str, float]` (Kleisli morphism A → Dist(B)) instead of `Optional[str]` — {} on miss; `LetStep.confidence` carries rule reliability into lambda terms; `eval_term` propagates accumulated Kleisli confidence through the let-chain (product of step confidences); all callers of `evaluate()` updated to handle distribution returns |
