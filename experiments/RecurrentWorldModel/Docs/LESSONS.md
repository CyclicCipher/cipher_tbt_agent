...
**Contraction ⊥ expressivity — the residual gate proves there's no single-block free lunch.** A LayerScale/ReZero gate on the attn+ffn residuals (`residual_gate`, small init) *does* make the forward converge — `gate_init=0.1` → **3 iters** vs never-in-200 ungated, confirming the map becomes `≈ norm(h + input)` (Jacobian ≈ 1/√dim). **But** the convergence is exquisitely gate-sensitive (0.1→3 iters, 0.2→59, 0.3→130, ≥0.5→never), and the band contractive enough for IFT (≤0.1) is too contractive to *compute*: at `gate_init=0.1` both IFT and BPTT barely clear chance in 300 steps (loss ~1.93–1.95) vs BPTT-no-gate at 1.64. So the gate *moves* the contraction/expressivity trade-off but cannot dissolve it — a small enough gate to converge fast leaves too little per-step computation. **This is structural, and it's exactly why "Solve the Loop" uses two modules:** an *expressive* backbone (computation) + a *contractive* attractor (fixed point). A single weight-tied operator cannot separate expressivity from contraction. **Consequence:** IFT at this scale needs either the two-module split, or accepting slow (100+ iter) convergence with a Broyden solver. **BPTT (no gate) stays the working recipe.**
