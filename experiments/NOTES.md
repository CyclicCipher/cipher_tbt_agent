...
## Sub-line: transformers → `experiments/transformers/`

The transformer work moved to its own folder on 2026-07-27, when it stopped being a one-off probe and became a research
line with a question of its own: **can a transformer be made as fast at acquiring a NEW skill as a bee?** Notes, results
and scripts live in `experiments/transformers/NOTES.md`.
