...
from .hippocampus import Hippocampus, world_facts, WorldMap, WorldModel
...
class Agent:
    ...
    def _world_facts(self, world=None) -> frozenset:
        """The COINCIDENCES in an allocentric world-state — the vocabulary win-condition hypotheses are written in.
        Derived from the world the hippocampus already holds, not generated by a rule about what a condition may be."""
        w = self.world_state() if world is None else world
        return world_facts(w, getattr(w, "kinds", {}))
    ...
    def _reachable_facts(self) -> frozenset:
        """Facts an IMAGINED reachable state could make true — where hypotheses come from. Not remembered episodes: on a
        fresh game none of them paid, so every hypothesis drawn from memory is refuted the moment it forms (measured, the
        Warehouse space emptied entirely). One step of each known action from here, through the learned model, which is
        what keeps every candidate satisfiable by construction."""
        world, model = self.world_state(), self.world_model()
        out = set(self._world_facts(world))
        for a in list(self._nav_col().operator._acc):          # the actions whose effect the operator has LEARNED
            try:
                out |= set(self._world_facts(model.step(world, a)))
            except Exception:
                continue
    ...
    def _live_hypotheses(self) -> set:
        ...
        return set(self.hippocampus.win.live(self._reachable_facts()))
    ...
    def _hypothesis_bits(self) -> float:
        ...
        return self.hippocampus.win.bits(self._reachable_facts())
    ...
    def _credit_goal(self, prev_objs, prev_pos, action, prev_cur, reward, sc) -> None:
        """Hand the hippocampus one OUTCOME: the coincidences that held, and whether it paid.

        `_winning_conditions` used to live here — a generator of `(mover, landmark)` candidates plus "the static the self
        reached", i.e. the rule grammar this project deleted once already and which survived under another name because it
        fed `goal_mem` rather than `_act`. It is gone. The vocabulary is now whatever coincidences the world-state
        actually contains, with nothing privileged: self, movers and landmarks all participate, and no rule says which
        pairs may be a condition. Proposal, refutation and generalisation are the hippocampus's
        (`hippocampus/hypotheses.py`), which is the region that binds self-location to what is where."""
        facts = self._world_facts()
        self.hippocampus.win.observe(facts, reward > 0)
        self.goal_mem.credit(facts, reward)          # the delta rule still SCORES coincidences, for drive salience
    ...
    def _goal_plan(self, objs, cur, pos, movement, conds=None):
        """Plan to a world-state in which the target COINCIDENCES all hold.

        One predicate for every kind of win condition, because a condition is now a fact about the world-state rather than
        a member of a hand-written grammar: `want <= facts(w)`. The old version case-analysed `(mover, landmark)` tuples
        and looked landmarks up in the feature-keyed `pos` map, which is why it could not express a win involving the body
        and crashed on anything that was not a pair.

        `None` when nothing is confirmed yet, or the target is UNREACHABLE under the learned model — saying so honestly is
        what lets the basal ganglia hand over to another drive instead of grinding at a goal it cannot get to."""
        want = self.hippocampus.win.confirmed if conds is None else frozenset(conds)
        if not want:
            return None
        return self.plan(lambda w: 1.0 if want <= self._world_facts(w) else 0.0, movement, horizon=32)
