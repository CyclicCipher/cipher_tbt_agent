...
Suite: **78 passed** (~18s; `test_column_arithmetic` is the ~16s end-to-end column test, the rest — `test_bg_thalamus`,
...
2. **The spatial column's remaining frontier.** DONE on this thread: L6a path integration (the operator), the L4↔L6a
   ...
   **the OPERATOR OVER OBJECT POSES** (`Column.dynamics` — an L5 engine fed by the poses `recognize` SOLVES, so perception →
   ...
   (a) **abstraction across GEOMETRY VARIANTS** — the state is still exact-geometry-keyed (`_quantise`), so the condition
   transfers only across the SAME relative geometry, not a bigger table lower down; the fix is overlap recall over relation
   SDRs (the `_key` refinement everywhere). And intrinsic features (colour/shape) are not yet cues, so the correlate
   demonstrated is RELATIONAL (a neighbour), not intrinsic. A WALL (partial-effect cue) is the same RW mechanism as support.
