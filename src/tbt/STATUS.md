...
Suite: **71 passed** (~18s; `test_column_arithmetic` is the ~16s end-to-end column test, the rest — `test_bg_thalamus`,
...
2. **The spatial column's remaining frontier.** DONE on this thread: L6a path integration (the operator), the L4↔L6a
   ...
   **the OPERATOR OVER OBJECT POSES** (`Column.dynamics` — an L5 engine fed by the poses `recognize` SOLVES, so perception →
   ...
   (a) **the context-gated OVERRIDE = gravity AND walls, ONE slice** (§9): the operator is the free kernel (everything falls /
   the push moves); a LOCAL RELATIONAL CONTEXT predicts the exception. The context now EXISTS (`relate`/`relation_of` —
   "supported" = a stable vertical displacement); the remaining work is the GATING (when the relation holds, suppress the
   kernel's prediction). A table stopping a fall and a wall stopping a push are ONE mechanism;
   (b) **the operator's KEY, DISCOVERED not given** — the caller names the action and the gate is read by hand; gravity's key
   is a CONDITION, and WHICH relation gates WHICH delta is the discovery problem. "Every object falls alike" is a hypothesis
   the world can refute (feathers), which today's operator states by keying on nothing.
