...
**SUBGOAL CHAINING BUILT 2026-07-28 — three real defects fixed, and Warehouse STILL 1/3.** Carried over from the
transformer line (`experiments/transformers`), where making chaining the INFERENCE-time computation rather than a training
signal took held-out composition from 0.046 to 0.363: composing *m* operations needs *m* applications, and TBT's *m* in
TASK space has to come from somewhere. `_task_subgoal` now proposes the next single push and the rollout achieves it, one
leg per step, so a whole Sokoban is a CHAIN whose every leg is in-distribution while the composite never was.
  * **Candidates are IMAGINED, not looked up.** It asked the task graph what had FOLLOWED from here, and that graph is
    learned strictly behind the agent — mean 0.02 learned successors of the state it stands in, so the drive was starved
    on ~215 steps out of 220. Now: one push of each DISCOVERED mover under each LEARNED action effect, generated by the
    forward model. No enumeration of subgoal kinds.
  * **The relation value is now a TD VALUE, not an immediate-reward fit.** `learn(..., done=True)` every step made it
    predict THIS step's reward, so hundreds of zero-reward steps drove every weight to zero and the one paying event was
    washed out — measured, **R held 0 positive bits even after a level was completed**, and the field the whole loop
    climbs was flat noise (~0.00–0.01). With bootstrapping it holds **48**.
  * **Infeasible pushes are filtered by the model's own prediction.** A crate imagined one cell along says nothing about
    whether it CAN go there; with two crates on a small board most single pushes are into a wall, an edge or the other
    crate. Measured before the filter: `_task_plan` returned None on EVERY step while subgoals were offered throughout.
  * **MEASURED FIELD, and the honest reach limit:** relation value by the block's distance from its pad is
    **0.999 / 0.832 / 0.666 / 0.499 / 0.499 / 0.499** for 0,1,2,3,4,6 cells — informative to THREE cells, flat beyond.
    Inside that reach each push gains a clean constant +0.167 and the chain climbs; outside it there is nothing to climb
    and the drive is correctly silent. That is the nav dead-zone `reference_eigenoptions_subgoals` names, with a number
    and a specific cause: the field reaches exactly as far as the relation code's overlap does.
  * ⚠ **AND IT DOES NOT CONVERT. Warehouse is 1/3 levels at 45 actions — identical to before the loop** (oracle 9/9/8),
