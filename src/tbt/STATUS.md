...
Suite: **60 passed** (~20s; `test_column_arithmetic` is the ~16s end-to-end column test, the rest — `test_bg_thalamus`,
...
online pose-solving (the same object recognised online, placed anywhere and entered mid-object). Rotation is tested
...
2. **The spatial column's remaining frontier.** DONE on this thread: L6a path integration (the operator), the L4↔L6a
   ...
   **R4 pose-invariant RECOGNITION** (the pose SOLVED from displacement geometry over a hypothesis population; free entry,
   ...
   mid-object `[-1,-1]` → `[0,0]`; L2/3's support-only `pool()` and the boundary's re-anchoring DELETED as superseded).
   What remains = **ROADMAP Phase 3b, OBJECT DYNAMICS + RELATIONS (ARCHITECTURE §9)** — inserted BEFORE the value/BG rework,
   because a critic with no forward model of OBJECTS has nothing to plan over. Its remaining steps, in order:
   (b) **the operator over OBJECT POSES — the next slice** (its input, the online `(object, pose)` stream, now exists).
   Track a recognised object across frames, learn `pose_t → pose_t+1`. Buys COMMON
