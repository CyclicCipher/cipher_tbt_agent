...
### ⛔ A ROUND TRIP THAT WAS DELETED — do not rebuild it (2026-07-31)

Four commits (`1d25623` → `6739e60`) built a task-region planning strand on top of this state and **all of it was reverted
here**, because every one of them reported the same result: **Warehouse 1/3, unchanged.** What was built and removed:

* `_imagined_configurations` — candidates one push away, generated by the forward model rather than looked up (the task
  graph had a mean of 0.02 learned successors of the state the agent stands in, so a lookup-based subgoal was starved).
* `_task_subgoal` re-pointed at those imagined candidates, and `_task_plan` given an explicit target.
* `_task_novelty` / `_task_explore` — an EPISTEMIC drive over configurations, and a 4th drive in `_act`.
* TD bootstrapping in `task_reward.learn`, plus two caches (`_bits_cache`, lazy `_task_R`) that existed only to pay for
  the cost of the above.
* The relation code widened `mw 3 → 11`: field reach 2 → 10 cells, **3.7× the cost, no Warehouse gain.**

**Cost, measured:** the suite went from ~110 s to **223 s**, well past the 2-minute rule. After reverting: **236 green in
112 s**. Nothing else regressed.

**WHY IT COULD NOT HAVE WORKED, which is the part worth keeping.** `_dynamics_delta("of", 6, …)` predicts **(0, 0)** — the
agent has actively LEARNED that crates do not move, because `_learn_dynamics` still keys off feature-based `_positions`
and drops the ambiguous colour. Object indexes (`7f770fb`) fixed that representation for mover DISCOVERY but never for
dynamics LEARNING. **No drive, however clever, can rescue a forward model that predicts no effect.** Start there, not at
the planner: the next attempt should re-key `_learn_dynamics` onto object indexes and re-measure Warehouse BEFORE adding
any machinery above it.

**The general lesson, since this is the second time:** four commits of planning machinery were added on top of a broken
forward model, each one reporting no gain, and none of them was reverted at the time. A step that does not move its
benchmark should be deleted in the same session it is measured (`feedback_decisive_full_cutover`).
