...
Suite: **128 passed** (~32s; `test_column_arithmetic` is the ~16s end-to-end column test, the rest — `test_bg_thalamus`,
...
2. **The spatial column's remaining frontier.** DONE on this thread: L6a path integration (the operator), the L4↔L6a
   ...
   **Step 1 (perception bridge) ✅ DONE (2026-07-18, `perceive.py`, `test_perceive`)** — the retina (`segment`) + the discovered
   self (`SelfTracker`, motion not colour), wired via `Agent.transduce`/`observe_self`/`self_color`. NEXT: **step 2** — the
   `Agent.step` loop on LockPath L0 (perceive → `world_state` → `plan` → act + `WorldModel.learn`/`learn_pose_move`/`learn_value`),
   solving one nav level end-to-end; then **step 3** — GOAL/REWARD from the sparse score (the deepest open piece) + novelty
   exploration; then Sokoban (the push, the rollout's strength), Tetris (multi-cell + rotation + the click action). Moving
   `decide`/`scan` OUT of the agent into the loop object is part of this (the THIN agent). Also open: the two-pane imagined-future
   WIDGET. Deferred within the hippocampus (DESIGN §4): theta/replay timing, graded remap.
