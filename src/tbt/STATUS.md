...
Suite: **51 passed** (~20s; `test_column_arithmetic` is the ~16s end-to-end column test, the rest — `test_bg_thalamus`,
`test_operator_path_integration`, `test_feature_at_location`, `test_l23_pooling`, `test_operator_se2`, `test_object_centric`,
`test_rotation_recognition` — are fast). Count history, 2026-07-15: 54 → **46** at the continuous cut-over (`test_oriented_grid`
+ `test_rotation_operator` deleted with the discrete-rotation code they covered) → **48** with R4's recognition tests → **51**
with R5's (the merge regression, the partial-view guard, and learning a rotated known object without duplicating it). Rotation
is now tested end-to-end through *recognition* — the property we actually wanted — rather than through the machinery that
implemented it. Run `python src/tests/test_reachability.py` for the wired map; the 20 legacy test files are archived under
...
2. **The spatial column's remaining frontier.** DONE on this thread: L6a path integration (the operator), the L4↔L6a
   ...
   **R4 pose-invariant RECOGNITION** (the pose SOLVED from displacement geometry over a hypothesis population; free entry,
   ...
   met at a novel pose is reinforced, not duplicated).
   What remains, in order:
   (a) the context-gated obstacle OVERRIDE (a wall blocks the move — read from local context; `operator.py`);
   (b) **SO(3)** — the state generalises unchanged; R4 removed the cubic scan (solving needs 3 non-collinear points, not an
   angular resolution), so this is now mostly `_solve` in 3-D;
   (c) the fully-unsupervised LEARNING-time boundary (TBT itself leaves it open — the caller still supplies the episode);
