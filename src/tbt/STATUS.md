...
Suite: **46 passed** (~18s; `test_column_arithmetic` is the ~16s end-to-end column test, the rest — `test_bg_thalamus`,
`test_operator_path_integration`, `test_feature_at_location`, `test_l23_pooling`, `test_operator_se2`, `test_object_centric`,
`test_rotation_recognition` — are fast). The count DROPPED from 54 at the 2026-07-15 cut-over: `test_oriented_grid` +
`test_rotation_operator` were deleted with the discrete-rotation code they covered (rotation is now tested end-to-end through
recognition, which is the property we actually wanted). Run `python src/tests/test_reachability.py` for the wired map; the 20
legacy test files are archived under `Legacy - DO NOT USE OR IMPORT!/tests/`.
...
and **ROTATION-invariant recognition** (an object learned at one orientation is recognised at a novel one, at ANY angle, with
the pose INFERRED and indistinguishable poses reported honestly, `test_rotation_recognition`). Each is minimal-but-real; each
...
2. **The spatial column's remaining frontier.** DONE on this thread: L6a path integration (the operator), the L4↔L6a
   ...
   **ROTATION invariance via the 2026-07-15 CONTINUOUS cut-over** (L6a's state is a continuous pose; the grid code is a
   read-out ⇒ SO(2) is exact at ANY angle, and the continuous-heading TENSOR form is MOOT — heading is just a float).
   What remains: the context-gated obstacle OVERRIDE (a wall blocks the move — read from local context; `operator.py`);
   **R4** — the union/evidence pooler, so recognition works when the sweep ENTERS anywhere (currently the replay assumes a
   shared anchor), giving ambiguity + symmetry as a POPULATION over poses rather than a tie list; **SO(3)** (the pose becomes
   a 3-DOF rotation — the state generalises, but the SCAN cost is cubic, so the union/evidence path is the prerequisite);
   the fully-unsupervised LEARNING-time boundary (TBT itself leaves it open — currently a minimal episode cue); the harder
   pooling test (objects that SHARE feature-at-location codes → genuine INCREMENTAL disambiguation via the union); and the SR
   (ROADMAP 3b) accumulating the operator into value. Cross-column VOTING over these identities is the thalamus's job (#3).
