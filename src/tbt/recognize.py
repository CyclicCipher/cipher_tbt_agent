"""Pose-invariant object recognition — the TBT/Monty evidence-based recogniser the 2019 grid-cell model lacked.
...
object is at its learned orientation." That is the gap a memorised `shape→pose` table papered over. This module is
the fix, faithful to the Thousand Brains Project's Monty (arXiv:2412.18354): an object is stored ONCE in its own
reference frame, and its pose is SOLVED for, not recalled — so a known object is recognised at an orientation never
seen, in CONTINUOUS space.

DOMAIN-GENERAL (the Danganronpa litmus): it consumes `(location, local-shape descriptor)` sensations and object
models — no grid, no colours, no game. The caller's perception turns a frame into objects + sensations.
...
is no per-orientation entry to learn wrong (the bug class the table allowed simply cannot exist here). Learning is
online + label-free: a freshly-segmented shape recognised as a rotation of a known object IS that object; otherwise
it is new (`add_if_novel`) — the object set is discovered by watching, never injected ([[feedback_bitter_lesson]]).
"""
