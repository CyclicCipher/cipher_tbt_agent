...
| **PFC / task column** | the GOAL-STATE + plan hierarchy (a column on an abstract task frame) | the same column interface, abstract frame | **BUILT 2026-07-27 (H2)** — `Column(graph=True)`, the SAME class fed the scene region's relational output, with a LEARNED graph L6a; the split is ANATOMICAL (it never receives position). Measured: 13 task states against 89 joint on LockPath, one task state abstracting over 35 agent positions. Read by nothing yet — no planner consults `state_value`. The old degenerate `Agent.task` was renamed `state_col` (it was a second column on the same transducer, read out for arithmetic's carry). **JUSTIFIED 2026-07-27 by H0** — the decision experiment the legacy `HETERARCHY_PLAN` gates everything on was run and one frame over the JOINT `(position, configuration)` state does NOT factorise: no shared position code, no shared operator, no transfer, and a multiplicative state count (live LockPath: 92 states = 40 cells × 7 configurations, 29 cells re-stored under a second configuration). Full numbers + why the eigen phrasing was not used: `STATUS.md`, `test_h0_factorisation`. ⇒ H1 (inter-column communication) then H2 (this column) stand. Open: WHO DECIDES THE SPLIT — H0's counterfactual arm split by hand, so it is a ceiling, not a mechanism |
